...
def test_create_task():
    r = requests.post('http://localhost:5050/v1/tasks', json={"title": "My First Task"})
    assert isinstance(r.json()["id"], int)
    assert len(r.json()) == 1

def test_list_all_tasks():
    r = requests.get('http://localhost:5050/v1/tasks')
...
def test_get_task():
    r = requests.get('http://localhost:5050/v1/tasks/1')
...
def test_update_task():
    r = requests.put('http://localhost:5050/v1/tasks/1', json={"title": "My 1st Task", "is_completed": True})
...
def test_delete_task():
    r = requests.delete('http://localhost:5050/v1/tasks/1')
    assert not r.content
...
def test_edit_missing_task():
    r = requests.put('http://localhost:5050/v1/tasks/1', json={})
    assert r.status_code == 404
...
def test_bulk_create_tasks():
    ...
    r = requests.post('http://localhost:5050/v1/tasks', json=data)
    res = r.json()
...
def delete_bulk_tasks():
    ...
    r = requests.delete('http://localhost:5050/v1/tasks', json=data)
    assert not r.content
...
test_get_task()
test_update_task()
test_get_updated_task()
test_delete_task()
test_get_missing_task()
test_edit_missing_task()
test_bulk_create_tasks()
test_list_all_tasks_again()
delete_bulk_tasks()
list_deleted_tasks()
